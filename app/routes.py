...
#             )
#         except Exception:
#             # 🔴 REAL LOGGING (before: swallowed)
#             current_app.logger.error(
#                 "Vanna connection failed",
#                 exc_info=True
#             )
#             result_reason = "Connect to database to run query"
...
#             except Exception:
#                 # 🔴 stack trace instead of message-only
#                 current_app.logger.error(
#                     "Error occurred while fetching driving table",
#                     exc_info=True
#                 )
...
#             )
#         except Exception:
#             # 🔴 THIS is where real Vanna errors show up
#             current_app.logger.error(
#                 "vn.ask() raised an exception",
#                 exc_info=True
#             )
#             sql, df, fig = None, None, None

#         # 🔴 Explicitly log silent failures
#         if sql is None:
#             current_app.logger.error(
#                 "Vanna returned sql=None (LLM refusal or internal failure)",
#                 extra={"query": vanna_query}
#             )

#         if df is None:
#             current_app.logger.error(
#                 "Vanna returned df=None",
#                 extra={"sql": sql}
#             )

#         # -------------------------------
#         # Retry loop (unchanged logic)
#         # -------------------------------
#         counter = 0
#         while (
#             isinstance(df, Exception)
#             and counter < 2
#         ):
#             current_app.logger.error(
#                 "Vanna run_sql error",
#                 exc_info=df if isinstance(df, Exception) else None
#             )
...
#             df_result = None
#             result_reason = f"SQL error: {df}"
#         elif df is None or getattr(df, "shape", [0])[0] == 0:
#             current_app.logger.info(
#                 "DataFrame is None or empty, no data returned from Vanna."
#             )
#             df_result = None
#         else:
#             current_app.logger.info(type(df))
...
#     except Exception:
#         current_app.logger.error(
#             "Unhandled exception in query_vanna",
#             exc_info=True
#         )
#         return jsonify({
