...
class Moderation(base_metric.BaseMetric):
    ...
    def _parse_model_output(self, content: str) -> score_result.ScoreResult:
        ...
        try:
            ...
            try:
                print("JSON decoding failed. Attempting to extract moderation with regex...")

                # Use regex to extract "score" and "reason"
                ...
                # Extract reason and clean it
                reasons_raw = reason_match.group(1)
                reasons_list = re.findall(r'["\'](.*?)["\']', reasons_raw)  # Extract individual items in reason list
                print("Extracted Moderation score amd reason with regex")

                return score_result.ScoreResult(
                    name=self.name,
                    value=score,
                    reason=str(reasons_list),
                )
            except Exception:
                raise exceptions.MetricComputationError(
                    logging_messages.MODERATION_SCORE_CALC_FAILED
